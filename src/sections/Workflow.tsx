...
import { PhoneCall, Calculator, Factory, Truck, ShieldCheck } from "lucide-react";
    ...
    <section id="workflow" className="bg-gray-50 py-16 sm:py-20">
      <div className="mx-auto max-w-container px-4">
        ...
        <p className="mx-auto mt-2 max-w-[65ch] text-center text-gray-600">
          От первого звонка до готового объекта — весь процесс прозрачен и понятен.
        </p>

        {/* Таймлайн */}
        <div className="relative mt-14 flex flex-col items-center sm:flex-row sm:justify-between">
          {/* линия */}
          <div className="absolute top-7 hidden h-[2px] w-full bg-gradient-to-r from-yellow-300 via-orange-400 to-orange-600 sm:block" />
          ...
          {steps.map((s, i) => (
            <motion.div
              ...
              className="relative z-10 mb-10 flex flex-col items-center text-center sm:mb-0 sm:w-1/5"
            >
              {/* иконка */}
              ...
              <p className="mt-2 max-w-[220px] text-sm text-gray-600">{s.text}</p>
            </motion.div>
          ))}
        </div>
      </div>
